...
use crate::{
    cost, dateparse, distances, logging, now, plot, random_fg_color, re_matches,
...
impl Screen {
    ...
    pub fn run(&mut self) {
        ...
        if let Some(first_node) = self.with_node(self.drawing_root, |n| n.children.get(0).cloned()).unwrap() {
    ...
    pub fn save(&self) {
        ...
        let data = crate::json_storage::serialize_screen(self);
